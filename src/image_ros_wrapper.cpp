...
void infrared_image_callback(const std::vector<uint8_t>& image_data, 
                             const ImageMetaData& metadata, 
                             image_transport::Publisher& pub)
{
    std::cout << "============= Infrared Image Callback =============" << std::endl;
...
int main(int argc, char** argv)
{
    ...
    image_transport::Publisher infrared_pub = it.advertise("camera/infrared/image", 1);

    UdpReceiver rgb_receiver(14601);
    UdpReceiver depth_receiver(14603);
    UdpReceiver infrared_receiver(14605);

    signal(SIGINT, signal_handler);

    optional<std::chrono::milliseconds> no_sleep_duration;

    std::thread rgb_thread(&UdpReceiver::start_receiving, &rgb_receiver, no_sleep_duration,
                           std::bind(rgb_image_callback, std::placeholders::_1,
                                     std::placeholders::_2, std::ref(rgb_pub)));

    std::thread depth_thread(&UdpReceiver::start_receiving, &depth_receiver, no_sleep_duration,
                             std::bind(depth_image_callback, std::placeholders::_1,
                                       std::placeholders::_2, std::ref(depth_pub)));

    std::thread infrared_thread(&UdpReceiver::start_receiving, &infrared_receiver, no_sleep_duration,
                                std::bind(infrared_image_callback, std::placeholders::_1,
                                          std::placeholders::_2, std::ref(infrared_pub)));
...
//     UdpReceiver rgb_receiver(14601);  // RGB数据的端口
//     UdpReceiver depth_receiver(14603);  // Depth数据的端口
